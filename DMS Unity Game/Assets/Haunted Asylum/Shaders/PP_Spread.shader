...
#include "Packages/com.unity.render-pipelines.high-definition/Runtime/PostProcessing/Shaders/RTUpscale.hlsl"
...

	{
		...
		//float4 _ghostCameraSpace = mul(_ghostPos, _Projection);

		_ghostPos.y = _screenY - _ghostPos.y; //Flip Y for some reason unity has Y fliped on world-screenspace

		//float4 _ghostScreenSpace = (_ghostCameraSpace * 0.5) + 0.5;
	...
	float3 _noiseTex = LOAD_TEXTURE2D(_Noise, _worldUV).xyz;
		...
		float worldDis = distance(float2(_ghostPos.x, _ghostPos.y), worldPos); //world dis floats determins where it will be positioned on screen place ghost position and convert it to screenspace
